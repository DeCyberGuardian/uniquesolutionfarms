...
import { getProductsData, getGalleryItems } from "@/lib/content";
...
export default async function ProductsPage() {
  ...
  // Pull a reasonable number for the grid; tweak as you like.
  const allGallery = await getGalleryItems(32);

  // Keep only images, then de-dupe by id|src so we never render duplicates.
  const galleryImages: GalleryItem[] = Array.from(
    new Map(
      (allGallery || [])
  ...
  return (
    <div className="min-h-screen">
          ...
          <div className="space-y-16">
            {productsData.categories.map((category, index) => (
              <div key={category.id} className="space-y-8">
                ...
                {/* Divider */}
                {index < productsData.categories.length - 1 && <div className="border-t border-border/50 pt-8" />}
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Brand / Process Gallery (images only, deduped) */}
      {galleryImages.length > 0 && (
        <section className="py-16 lg:py-24 bg-muted/30">
          <div className="container mx-auto px-4">
            <div className="text-center mb-10">
              <h2 className="font-display text-3xl md:text-4xl font-bold text-foreground mb-4">From Farm to Bottle</h2>
              <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
                A look at our farming, processing, and packaging journey.
              </p>
            </div>

            <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {galleryImages.map((g, idx) => (
                <figure key={`prod-gallery-${g.id}-${idx}`} className="relative aspect-square rounded-2xl overflow-hidden shadow">
                  <Image
                    src={g.src}
